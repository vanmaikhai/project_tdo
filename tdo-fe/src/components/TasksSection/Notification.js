...
const Notification = () => {
    ...
    const { notificationIsVisible, showNotifications } = useVisibility();
    ...
    return (
        <div className="sm:mr-4 md:mr-6 ml-auto grid place-items-center relative">
            <button
                ...
                <IconBell className="fill-amber-600 w-5 h-5 md:w-6 md:h-6 dark:fill-amber-800" />
            </button>
            {notificationIsVisible && (
                <div className="absolute bg-slate-100 dark:bg-slate-800 top-full rounded-md right-0 p-3 w-96 border border-slate-300 dark:border-slate-700">
