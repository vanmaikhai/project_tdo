    ...
    return (
        <>
            {todaysTasks.length !== 0 && (
                <div className="mt-8">
                    <span className="flex justify-between mb-2">
                        <span>{t('task_today')}</span> {todayTasksDone.length}/{todaysTasks.length}
                    </span>
            ...
            {tasks.length !== 0 && (
                <div className="mt-6">
                    <span className="flex justify-between mb-2">
                        <span>{t('all_tasks')} </span> {allTasksDone.length}/{tasks.length}
                    </span>
                    <div className="barProgress">
            ...
            {todaysTasks.length === 0 && (
                <span className="mt-6 block pt-4 border-t-slate-200 dark:border-t-slate-700/[.3] border-t-2">No tasks today</span>
            )}

            {todaysTasks.length > 0 && (
                <div className="mt-8">
                    <span className="mb-2 block">{t('today_tasks')}</span>
                    <ul>
